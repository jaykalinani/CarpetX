...
CalcYfFromKcs(const Loop::GridDescBaseDevice &grid,
  ...
  // amrex::lbound(cf_mask) is the global index of that first allocated cell.
  const auto cf_mask_lo = amrex::lbound(cf_mask);
  const int cf_ox = cf_mask_lo.x;
  const int cf_oy = cf_mask_lo.y;
  const int cf_oz = cf_mask_lo.z;
